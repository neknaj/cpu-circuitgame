...
test_func_8x1_8("buf8bits",u8ToBoolArray,(x)=>x.map(y=>y));
test_func_8x1_8("not8bits",u8ToBoolArray,(x)=>x.map(y=>!y));
test_func_8x2_8("and8bits",u8ToBoolArray,(x)=>groupArray(x).map((b)=>b[0]&&b[1]));
test_func_8x2_8("or8bits",u8ToBoolArray,(x)=>groupArray(x).map((b)=>b[0]||b[1]));
test_func_8x2_8("xor8bits",u8ToBoolArray,(x)=>groupArray(x).map((b)=>b[0]^b[1]?true:false));

test_func_8x2_8("add8bits",u8ToBoolArray,(x)=>u8ToBoolArray(boolArrayToU8(x.slice(0,8))+boolArrayToU8(x.slice(8,16))));
test_func_8x2_8("sub8bits",u8ToBoolArray,(x)=>u8ToBoolArray(boolArrayToU8(x.slice(0,8))-boolArrayToU8(x.slice(8,16))));

test_func_8x2_8("increment8bits",u8ToBoolArray,(x)=>u8ToBoolArray(boolArrayToU8(x.slice(0,8))+1));
test_func_8x2_8("decrement8bits",u8ToBoolArray,(x)=>u8ToBoolArray(boolArrayToU8(x.slice(0,8))-1));
...
function test_func_8x1_8(name: string, fn1: (x:number)=>FixedLengthArray<boolean, 8>, ref: (x: FixedLengthArray<boolean, 8>)=>FixedLengthArray<boolean, 8>) {
    ...
    for (let a=0x00;a<0xff;a++) {
        if (!arrEqual( Modules[name].func(...fn1(a)), ref([...fn1(a)]) )) {
            failed = true;
            console.error(`Test: `,a,Modules[name].func(...fn1(a)).map(x=>x?"t":"f").join(""),ref([...fn1(a)]).map(x=>x?"t":"f").join(""))
        }
    }
...
function test_func_8x2_8(name: string, fn1: (x:number)=>FixedLengthArray<boolean, 8>, ref: (x: FixedLengthArray<boolean, 16>)=>FixedLengthArray<boolean, 8>) {
    ...
    for (let a=0x00;a<0xff;a++) {
        for (let b=0x00;b<0xff;b++) {
            if (!arrEqual( Modules[name].func(...fn1(a),...fn1(b)), ref([...fn1(a),...fn1(b)]) )) {
                failed = true;
                console.error(`Test: `,a,b,Modules[name].func(...fn1(a),...fn1(b)).map(x=>x?"t":"f").join(""),ref([...fn1(a),...fn1(b)]).map(x=>x?"t":"f").join(""))
            }
        }
    }
    console.log(`Test: ${name}`,styleText(...(!failed?["blueBright","ok"]:["redBright","error"])))
